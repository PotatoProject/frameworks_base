...
public class QuickStatusBarHeader extends RelativeLayout implements
    ...
    private void updateResources() {
        ...
        updateOreoBatteryColor();
        updateStatusIconAlphaAnimator();
        updateHeaderTextContainerAlphaAnimator();
    }

    private void updateOreoBatteryColor() {
        Rect tintArea = new Rect(0, 0, 0, 0);
        int colorForeground = Utils.getColorAttr(mContext, android.R.attr.colorForeground);
        float intensity = getColorIntensity(colorForeground);
        int fillColor = fillColorForIntensity(intensity, mContext);

        mOreoBatteryMeterView.setColorsFromContext(mContext);
        mOreoBatteryMeterView.onDarkChanged(new Rect(), 0, fillColor);
    }
