...
public class CollapsedStatusBarFragment extends Fragment implements CommandQueue.Callbacks {
    ...
    private static final int HIDE_DELAY = 60*1000; // 1 minute
    private static final int SHOW_DELAY = 5*1000; // 5 seconds
    ...
    private void setupSmartClock(boolean animate) {
        if (useSmartClock) {
            displaySmartClock(animate);
            smartClockHandler.postDelayed(runnable = new Runnable() {
                public void run() {
                    displaySmartClock(animate);
                    smartClockHandler.postDelayed(runnable, HIDE_DELAY);
                }
            }, HIDE_DELAY);
        }
    }
    ...
    private void displaySmartClock(boolean animate) {
        ...
        if (useSmartClock) {
            ...
            handler.postDelayed(new Runnable() {
                public void run() {
                   // Always animate hiding of clock here
                   hideClock(true);
                }
            }, SHOW_DELAY);
        }
